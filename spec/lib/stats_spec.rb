# frozen_string_literal: true

require_relative "../plugin_helper"

describe ProcessManager::Stats do
  fab!(:process) { Fabricate(:process, name: "Stats Process") }
  fab!(:category_1, :category)
  fab!(:category_2, :category)
  fab!(:step_1) do
    Fabricate(:process_step, process_id: process.id, category_id: category_1.id, position: 1)
  end
  fab!(:step_2) do
    Fabricate(:process_step, process_id: process.id, category_id: category_2.id, position: 2)
  end
  fab!(:topic_1) { Fabricate(:topic, category: category_1) }
  fab!(:topic_2) { Fabricate(:topic, category: category_1) }
  fab!(:state_1) do
    Fabricate(
      :process_state,
      topic_id: topic_1.id,
      process_id: process.id,
      process_step_id: step_1.id,
    )
  end
  fab!(:state_2) do
    Fabricate(
      :process_state,
      topic_id: topic_2.id,
      process_id: process.id,
      process_step_id: step_2.id,
    )
  end

  it "calculates daily counts per process step from process states" do
    SiteSetting.process_manager_enabled = true

    expect do described_class.new.calculate_daily_stats end.to change {
      ProcessManager::ProcessStat.count
    }.by(2)
  end

  it "ignores corrupted states with null process_id or process_step_id" do
    SiteSetting.process_manager_enabled = true

    state_1.update_columns(process_id: nil)
    state_2.update_columns(process_step_id: nil)

    valid_topic = Fabricate(:topic, category: category_1)
    Fabricate(
      :process_state,
      topic_id: valid_topic.id,
      process_id: process.id,
      process_step_id: step_1.id,
    )

    described_class.new.calculate_daily_stats

    today_stats = ProcessManager::ProcessStat.where(cob_date: Date.current)
    expect(today_stats.count).to eq(1)
    expect(today_stats.first.process_id).to eq(process.id)
    expect(today_stats.first.process_step_id).to eq(step_1.id)
    expect(today_stats.first.count).to eq(1)
  end
end
